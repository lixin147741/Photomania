...
class PhotoViewerViewController: UIViewController, UIScrollViewDelegate, UIPopoverPresentationControllerDelegate, UIActionSheetDelegate {
  ...
  override func viewDidLoad() {
    ...
    //增加向右滑动返回主界面
    let guesture = UIScreenEdgePanGestureRecognizer(target: self, action: "goBack")
    self.view.addGestureRecognizer(guesture)
    
    //长按显示ActionSheet
    let longTapGuesture = UILongPressGestureRecognizer(target: self, action: "showActions")
    self.view.addGestureRecognizer(longTapGuesture)
  }
    ...
    func loadPhoto() {
        Alamofire.request(Five100px.Router.PhotoInfo(self.photoID, .Large)).validate().responseObject { (_, _, photoInfo: PhotoInfo?, error) -> Void in
            ...
            if error == nil {
                ...
                println("photo id \(self.photoInfo!.url)")
  ...
  func downloadPhoto() {
    Alamofire.request(Five100px.Router.PhotoInfo(photoInfo!.id, .XLarge)).validate().responseJSON { (_, __, JSON, error) -> Void in
        ...
        if error == nil {
            ...
            let destination: (NSURL, NSHTTPURLResponse) -> NSURL = {
                ...
                if let dictionaryURL = NSFileManager.defaultManager().URLsForDirectory( NSSearchPathDirectory.DocumentDirectory, inDomains: NSSearchPathDomainMask.UserDomainMask)[0] as? NSURL
                {
                    return dictionaryURL.URLByAppendingPathComponent("\(self.photoInfo!.id).\(response.suggestedFilename)")
                }
                return temporaryURL
            }
